...
public class IntegralGoodsLogic {
	...
	private static void parseIntegralGoodsListData(JSONObject response,
		...
		try {
			...
			if (sucResult.equals(MsgResult.RESULT_SUCCESS)) {
				...
				JSONArray integralGoodsListArray = jsonObject
				...
				for (int i = 0; i < size; i++) {
					IntegralGoods integralGoods = (IntegralGoods) JsonUtils
							.fromJsonToJava(
									integralGoodsListArray.getJSONObject(i),
									IntegralGoods.class);
	...
	private static void parseIntegralGoodsRangeData(JSONObject response,
			Handler handler) {
		try {
			String sucResult = response.getString(MsgResult.RESULT_TAG).trim();
			if (sucResult.equals(MsgResult.RESULT_SUCCESS)) {
	...
	public static void exchange(final Context context, final Handler handler,
			final String phone, final String productId, final String address,
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					requestJson.put("receiverName",
							URLEncoder.encode(receiverName, "UTF-8"));
	...
	public static void getExchangeHistroy(final Context context,
			final Handler handler, final String phone, final String productId) {
	...
	public static void getTotal(final Context context, final Handler handler,
			final String phone) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					SoapObject rpc = new SoapObject(RequestUrl.NAMESPACE,
							RequestUrl.integral.queryTotal);
					...
					ht.call(RequestUrl.NAMESPACE + "/"
							+ RequestUrl.integral.queryTotal, envelope);
	...
	public static void getExchangeOrder(final Context context,
			final Handler handler, final String phone, final String pageNum,
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					SoapObject rpc = new SoapObject(RequestUrl.NAMESPACE,
							RequestUrl.integral.queryExchangeOrder);
					...
					ht.call(RequestUrl.NAMESPACE + "/"
							+ RequestUrl.integral.queryExchangeOrder, envelope);
	...
	private static void parseExchangeOrderData(JSONObject response,
			Handler handler) {
		try {
			...
			if (sucResult.equals(MsgResult.RESULT_SUCCESS)) {
				...
				JSONObject jsonObject = response
						.getJSONObject(MsgResult.RESULT_DATAS_TAG);
				ArrayList<IntegralGoods> mTempGoodsList = new ArrayList<IntegralGoods>();
				JSONArray goodsListArray = jsonObject
						.getJSONArray(MsgResult.RESULT_LIST_TAG);
				Log.e("xxx_parseGoodsListData_0", "goodsListArray-------->"
						+ goodsListArray.toString());
				int size = goodsListArray.length();
				for (int j = 0; j < size; j++) {
					JSONObject integralGoodsJsonObject = goodsListArray
							.getJSONObject(j);

					IntegralGoods integralGoods = (IntegralGoods) JsonUtils.fromJsonToJava(
							integralGoodsJsonObject, IntegralGoods.class);
					mTempGoodsList.add(integralGoods);
				}
				Message message = new Message();
