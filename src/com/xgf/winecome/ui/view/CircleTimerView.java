...
public class CircleTimerView extends View {
	...
	private void initialize() {
		...
		gapBetweenCircleAndLine = TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP,
				DEFAULT_GAP_BETWEEN_CIRCLE_AND_LINE, getContext()
						.getResources().getDisplayMetrics());
		gapBetweenNumberAndLine = TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP,
				DEFAULT_GAP_BETWEEN_NUMBER_AND_LINE, getContext()
						.getResources().getDisplayMetrics());
		numberSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
				DEFAULT_NUMBER_SIZE, getContext().getResources()
						.getDisplayMetrics());
		lineLength = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
				DEFAULT_LINE_LENGTH, getContext().getResources()
						.getDisplayMetrics());
		longerLineLength = TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, DEFAULT_LONGER_LINE_LENGTH,
				getContext().getResources().getDisplayMetrics());
		lineWidth = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
				DEFAULT_LINE_WIDTH, getContext().getResources()
						.getDisplayMetrics());
		circleButtonRadius = TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, DEFAULT_CIRCLE_BUTTON_RADIUS,
				getContext().getResources().getDisplayMetrics());
		circleStorkeWidth = TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, DEFAULT_CIRCLE_STROKE_WIDTH,
				getContext().getResources().getDisplayMetrics());
		shadowRadius = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
				DEFAULT_CIRCLE_BUTTON_SHADOW_RADIUS, getContext()
						.getResources().getDisplayMetrics());
		shadowXOffset = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
				DEFAULT_CIRCLE_BUTTON_SHADOW_X_OFFSET, getContext()
						.getResources().getDisplayMetrics());
		shadowYOffset = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
				DEFAULT_CIRCLE_BUTTON_SHADOW_Y_OFFSET, getContext()
						.getResources().getDisplayMetrics());
		timerNumberSize = TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, DEFAULT_TIMER_NUMBER_SIZE,
				getContext().getResources().getDisplayMetrics());
		timerTextSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
				DEFAULT_TIMER_TEXT_SIZE, getContext().getResources()
						.getDisplayMetrics());
		gapBetweenTimerNumberAndText = TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP,
				DEFAULT_GAP_BETWEEN_TIMER_NUMBER_AND_TEXT, getContext()
						.getResources().getDisplayMetrics());
		...
		circleButtonPaint.setShadowLayer(shadowRadius, shadowXOffset,
		...
		// setLayerType(View.LAYER_TYPE_SOFTWARE, null); // use this, when
		// targetSdkVersion is greater than or equal to api 14
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < 120; i++) {
			...
			if (i % 30 == 0) {
				if (360 / 120 * i <= Math.toDegrees(currentRadian)) {
					canvas.drawLine(cx, getMeasuredHeight() / 2 - radius
							+ circleStorkeWidth / 2 + gapBetweenCircleAndLine,
							cx, getMeasuredHeight() / 2 - radius
									+ circleStorkeWidth / 2
									+ gapBetweenCircleAndLine
									+ longerLineLength, highlightLinePaint);
				} else {
					canvas.drawLine(cx, getMeasuredHeight() / 2 - radius
							+ circleStorkeWidth / 2 + gapBetweenCircleAndLine,
							cx, getMeasuredHeight() / 2 - radius
		...
		canvas.drawText("60", cx, getMeasuredHeight() / 2 - radius
				+ circleStorkeWidth / 2 + gapBetweenCircleAndLine
				+ longerLineLength + gapBetweenNumberAndLine
				+ getFontHeight(numberPaint), numberPaint);
		canvas.drawText("15", cx + radius - circleStorkeWidth / 2
				- gapBetweenCircleAndLine - longerLineLength - textLength / 2
				- gapBetweenNumberAndLine, cy + getFontHeight(numberPaint) / 2,
				numberPaint);
		canvas.drawText("30", cx, getMeasuredHeight() / 2 + radius
				- circleStorkeWidth / 2 - gapBetweenCircleAndLine
				- longerLineLength - gapBetweenNumberAndLine, numberPaint);
		canvas.drawText("45", getMeasuredHeight() / 2 - radius
				+ circleStorkeWidth / 2 + gapBetweenCircleAndLine
				+ longerLineLength + gapBetweenNumberAndLine + textLength / 2,
				cy + getFontHeight(numberPaint) / 2, numberPaint);
		// Circle button
		canvas.save();
		canvas.rotate((float) Math.toDegrees(currentRadian), cx, cy);
		canvas.drawCircle(cx, getMeasuredHeight() / 2 - radius
				+ circleStorkeWidth / 2 + gapBetweenCircleAndLine + lineLength
				/ 2, circleButtonRadius, circleButtonPaint);
		canvas.restore();
		// TimerNumber
		canvas.save();
		canvas.drawText((currentTime / 60 < 10 ? "0" + currentTime / 60
				: currentTime / 60)
				+ " "
				+ (currentTime % 60 < 10 ? "0" + currentTime % 60
						: currentTime % 60), cx, cy
				+ getFontHeight(timerNumberPaint) / 2, timerNumberPaint);
		canvas.drawText(":", cx, cy + getFontHeight(timerNumberPaint) / 2,
				timerColonPaint);
		canvas.restore();
		// Timer Text
		canvas.save();
		canvas.drawText("时间设置", cx, cy + getFontHeight(timerNumberPaint) / 2
				+ gapBetweenTimerNumberAndText + getFontHeight(timerTextPaint)
				/ 2, timerTextPaint);
		canvas.restore();
		super.onDraw(canvas);
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			// if (preRadian > Math.toRadians(270)
			// && temp < Math.toRadians(90)) {
			// preRadian -= 2 * Math.PI;
			// } else if (preRadian < Math.toRadians(90)
	...
	private boolean isInCircleButton(float x, float y) {
		float r = radius - circleStorkeWidth / 2 - gapBetweenCircleAndLine
				- lineLength / 2;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (lineLength / 2 + gapBetweenCircleAndLine + circleStorkeWidth >= (circleButtonRadius + shadowRadius)) {
			...
			this.radius = width
					/ 2
					- (circleButtonRadius + shadowRadius
							- gapBetweenCircleAndLine - lineLength / 2 - circleStorkeWidth / 2);
			Log.d(TAG, "Exceed");
		}
