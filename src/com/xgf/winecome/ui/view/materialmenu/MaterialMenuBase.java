...
public abstract class MaterialMenuBase implements MaterialMenu {
	...
	public MaterialMenuBase(Activity activity, int color,
	...
	public MaterialMenuBase(Activity activity, int color,
			MaterialMenuDrawable.Stroke stroke, int transformDuration) {
		this(activity, color, stroke, transformDuration,
				DEFAULT_PRESSED_DURATION);
	}
	...
	public MaterialMenuBase(Activity activity, int color,
			MaterialMenuDrawable.Stroke stroke, int transformDuration,
			int pressedDuration) {
		drawable = new MaterialMenuDrawable(activity, color, stroke,
				DEFAULT_SCALE, transformDuration, pressedDuration);
	...
	private void setupActionBar(Activity activity) {
		...
		ViewGroup.MarginLayoutParams upParams = (ViewGroup.MarginLayoutParams) upView
				.getLayoutParams();
		upParams.leftMargin = activity.getResources().getDimensionPixelSize(
				R.dimen.mm_up_arrow_margin);
	...
	@Override
	public final void setTransformationOffset(
			MaterialMenuDrawable.AnimationState animationState, float value) {
		currentState = getDrawable().setTransformationOffset(animationState,
				value);
	}
