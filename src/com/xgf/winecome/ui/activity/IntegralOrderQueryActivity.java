...
public class IntegralOrderQueryActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mContext = IntegralOrderQueryActivity.this;
		setUpViews();
		setUpListener();
		setUpData();
		// initQuerySmsInbox();
	...
	private void initQuerySmsInbox() {
		SmsContent content = new SmsContent(IntegralOrderQueryActivity.this, new Handler(), mVerCodeEt);
		// 注册短信变化监听
		...
		case R.id.integral_query_submit_ll: {
			if (!mIsNeedAuth) {
				if (!TextUtils.isEmpty(mPhone)) {
					Intent intent = new Intent(IntegralOrderQueryActivity.this, IntegralOrderListActivity.class);
					intent.putExtra("phone", mPhone);
					startActivity(intent);
					IntegralOrderQueryActivity.this.finish();
